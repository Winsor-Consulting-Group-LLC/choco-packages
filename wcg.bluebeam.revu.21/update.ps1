...
function global:au_GetLatest {
    ...
    if (-not(Test-Path $revuInstallerPath)) { 
    ...
    $revuInstallerHash = (Get-FileHash -Path $revuInstallerPath -Algorithm SHA256).Hash
    # $ocrInstallerHash  = (Get-FileHash -Path $ocrInstallerPath -Algorithm SHA256).Hash

    # Copy-Item "$filepath\Bluebeam Revu x64 21.msi" "$toolpath\Bluebeam Revu x64 21.msi"

    $installerPath    = Get-Item -Path $revuInstallerPath
    $installerPath64  = Get-Item -Path $revuInstallerPath
    $installerUrl     = [Uri]::EscapeUriString($revuInstallerUrl)
    $installerUrl64   = [Uri]::EscapeUriString($revuInstallerUrl)
    $installerHash    = $revuInstallerHash
    $installerHash64  = $revuInstallerHash
    return @{
        URL32           = $installerUrl
